...
VITE_HOME_TOPIC_ANCHOR_MIN_PATHS="${VITE_HOME_TOPIC_ANCHOR_MIN_PATHS:-1}"
VITE_HOME_NEW_MAX_AGE_DAYS="${VITE_HOME_NEW_MAX_AGE_DAYS:-7}"
...
DOCKER_BUILDKIT=1 docker build --platform "$PLATFORM" -t "$IMAGE_FRONTEND" -f "$FRONTEND_DIR/Dockerfile" \
  ...
  --build-arg VITE_HOME_TOPIC_ANCHOR_MIN_PATHS="$VITE_HOME_TOPIC_ANCHOR_MIN_PATHS" \
  --build-arg VITE_HOME_NEW_MAX_AGE_DAYS="$VITE_HOME_NEW_MAX_AGE_DAYS" \
  "$FRONTEND_DIR"
